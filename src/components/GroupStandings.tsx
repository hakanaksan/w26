      ...
      <div className="flex gap-2 mb-6 overflow-x-auto pb-2">
        {groups.map(group => (
          <button key={group.id} onClick={() => onGroupChange(group.id)}
            className={`px-5 py-2.5 rounded-xl text-sm font-semibold transition-all ${selectedGroup === group.id ? 'bg-blue-600 text-white shadow-lg shadow-blue-200' : 'bg-white text-gray-600 hover:bg-gray-50 border border-gray-200'}`}>
            {group.name}
          </button>
        ))}
      </div>
          ...
          <table className="w-full">
            ...
            <tbody>
              {calculateStandings(selectedGroup).map((standing, index) => {
                ...
                return (
                  <tr key={standing.teamId} className={`border-b border-gray-100 transition-colors hover:bg-gray-50 ${isQualified ? 'bg-emerald-50/50' : ''}`}>
                    <td className="py-4 px-4">
                      <div className={`w-7 h-7 rounded-lg flex items-center justify-center text-sm font-bold ${isQualified ? 'bg-emerald-100 text-emerald-700' : 'bg-gray-100 text-gray-500'}`}>{index + 1}</div>
                    </td>
<td className="py-4 px-4">
                       <Link href={`/team/${standing.teamId}`} className="flex items-center gap-3 group">
                         ...
                         <span className="font-semibold text-gray-900 group-hover:text-blue-600 transition-colors">{team.name}</span>
                       </Link>
                     </td>
                    <td className="text-center py-4 px-2 text-gray-600 font-medium">{standing.played}</td>
                    <td className="text-center py-4 px-2 text-gray-600">{standing.won}</td>
                    <td className="text-center py-4 px-2 text-gray-600">{standing.drawn}</td>
                    <td className="text-center py-4 px-2 text-gray-600">{standing.lost}</td>
                    <td className="text-center py-4 px-2 text-gray-600">{standing.goalsFor}</td>
                    <td className="text-center py-4 px-2 text-gray-600">{standing.goalsAgainst}</td>
                    <td className="text-center py-4 px-2 font-medium"><span className={standing.goalDifference > 0 ? 'text-emerald-600' : standing.goalDifference < 0 ? 'text-red-600' : 'text-gray-500'}>{standing.goalDifference > 0 ? '+' : ''}{standing.goalDifference}</span></td>
                    <td className="text-center py-4 px-4"><span className="text-lg font-black text-blue-600">{standing.points}</span></td>
                  </tr>
                );
              })}
            </tbody>
      ...
      <div className="mt-4 flex items-center gap-6 text-sm text-gray-600">
        <div className="flex items-center gap-2"><div className="w-4 h-4 bg-emerald-100 rounded-lg border border-emerald-200" /> <span>Son 32'ye yükselir</span></div>
      </div>
    </div>
  );
}